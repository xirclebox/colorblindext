...
// Get the filter target element.
// Firefox does not reliably apply CSS filter: url(#id) to the <html> element,
// so we target <body> instead. This works consistently across both browsers.
...
function applyFilters(colorFilter, lowVisionFilter) {
  ...
  // CSS filter on <body>
  // Applying to <body> rather than <html> ensures Firefox honours the
  // SVG fragment-identifier url(#id) reference correctly.
  if (!hasColor && !lvIsSVG) {
    // Nothing active — clear everything
    target.style.filter = "";
  } else if (hasColor && !lvIsSVG) {
    // Color only
    target.style.filter = `url(#${colorFilter})`;
  } else if (!hasColor && lvIsSVG) {
    // Low vision SVG only
    target.style.filter = `url(#${lowVisionFilter})`;
  } else {
    // Both: build a combined filter and apply it
    buildCombinedSVGFilter(colorFilter, lowVisionFilter);
    target.style.filter = "url(#vs-combined)";
  }
}
