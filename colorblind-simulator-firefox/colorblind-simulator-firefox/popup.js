...
// Firefox also aliases `chrome`, but using `browser` directly avoids edge-case
...
// Creates a <div class="cb__badge"> with safe text content
...
// Creates a <ul class="cb__active-filters__list"> containing one <li
// class="cb__badge"> per label
...
const activeFilters = () => {
  ...
  if (activeColorFilter === "none" && activeLowVisionFilter === "none") {
    ...
    activeFilterContainer.appendChild(makeBadgeList("Trichromacy", activeLowVisionFilter));
  } else {
    activeFilterContainer.appendChild(makeBadgeList(activeColorFilter, activeLowVisionFilter));
  }
...
// Sends a message to a tab and returns a Promise that resolves with the
// response, or resolves with null on timeout / error (never rejects).
...
// restore saved state
api.storage.local.get(["colorFilter", "lowVisionFilter"]).then((result) => {
  ...
  return sendMessageToTab(tabs[0].id, { action: "getFilters" }).then((response) => {
    if (!response) return;

    let changed = false;

    if (response.colorFilter && response.colorFilter !== activeColorFilter) {
      activeColorFilter = response.colorFilter;
      changed = true;
    }

    if (response.lowVisionFilter && response.lowVisionFilter !== activeLowVisionFilter) {
